...
pub fn tell<W: Monoid + Send>(w: W) -> impl WriterEffect<Output = (), Writes = W, Error = !, Env = ()>
```
...
pub fn tell_one<T: Send>(item: T) -> impl WriterEffect<Output = (), Writes = Vec<T>, Error = !, Env = ()>
```
...
##### FR-4.2: `run_writer_only` - Execute and Discard Writes

```rust
/// Execute the effect, discarding accumulated writes (for testing).
...
- [ ] **AC16**: `Vec<T>` works as writes type
- [ ] **AC17**: `String` works as writes type
- [ ] **AC18**: Custom monoid types work
- [ ] **AC19**: `Sum<i32>` works for numeric accumulation (counts, metrics)

### Error Handling

- [ ] **AC20**: On error, accumulated writes up to error point are returned
- [ ] **AC21**: `or_else` recovery preserves original writes
...
impl<W: Monoid + Send + Clone> Effect for Tell<W> {
    type Output = ();
...
impl<W: Monoid + Send + Clone> WriterEffect for Tell<W> {
    type Writes = W;
...
pub fn tell<W: Monoid + Send + Clone>(w: W) -> Tell<W> {
    Tell { writes: w }
}

/// Emit a single item to a Vec accumulator.
pub fn tell_one<T: Send>(item: T) -> Tell<Vec<T>> {
    Tell { writes: vec![item] }
}
```
